...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        [HttpDelete]
        [Route("del")]
        public async Task<IActionResult> DeleteDevices(DeleteDeviceRequest request)
        {
            var devices = await _devices.GetDeviceByName(request.NameDel);
            if (devices == null)
                return StatusCode(400, $"Ошибка: устройство {request.NameDel} не подключено. Сначала подключите устройство!!");
           
            await _devices.DeleteDevice(devices);

            return StatusCode(200, $"Устройство {request.NameDel} удалено.");
